...
const InstituteDashboard = () => {
  ...
  const instituteId = sessionStorage.getItem('coordinatorid');

  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/stats/institute/${instituteId}`)
      .then((response) => {
  ...
  return (
    <Container fluid style={{ padding: '20px' }}>
      ...
      <div className="text mb-4">
        ...
        <p style={{ fontSize: '16px', color: '#333' }}>View your institute's overall statistics.</p>
      </div>
      ...
      <Row className="mt-4">
        ...
        <Col md={6}>
          <Card className="p-3 shadow-sm">
            <Card.Body>
              <Card.Title className="fw-bold text-center mb-3">Institute Statistics Overview</Card.Title>
